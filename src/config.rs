...
const PROMPT_CLEAN: &str = "你是语音转写清理器。任务:1) 删除口语填充词与应答词(中文:嗯、啊、呃、\
唉、那个、然后那个、就是说、这个这个等;英文:uh、um、er、yeah、yep、ok、okay、mm、mhm 等)与无意义的\
重复字词;2) 结合上下文改正错别字、同音字和明显的识别错误(把听错写错的字词改成正确的),并补全正确\
标点。除此之外保留原话的词序与内容,不要改写、不要合并、不要总结、不要回答其中的问题。\
